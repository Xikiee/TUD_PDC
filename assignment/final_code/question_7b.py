...
EP_array=EP_SS1*np.ones(len(times))

for i,t in enumerate(times):
  if 20<t<=30:
    EP_array[i]=EP_array[i-1]+40
  elif 90<t<=100:
    EP_array[i]=EP_array[i-1]-40
  else:
...
plt.plot(t_min, EP_series)
plt.xlim(0, 200); plt.xticks(np.arange(0, 201, 20))
plt.ylim(0, 600); plt.yticks(np.arange(0, 601, 100))
plt.xlabel("Time (min)"); plt.ylabel("Power (kW)"); plt.title("Figure 1: EP profile")
plt.grid(True, linestyle='-', alpha=0.6)
...
plt.xlim(0, 200); plt.xticks(np.arange(0, 201, 20))
plt.xlabel("Time (min)"); plt.ylabel("Temperature (C0)"); plt.title("Figure 2: T1 & T3 Profiles")
...
plt.figure(figsize=(7,4))
plt.plot(t_min, np.full_like(t_min, F5_const), label="F5 (const)")
plt.plot(t_min, F3H2_series, label="F3H2 = p2·EP")
plt.xlim(0, 200); plt.xticks(np.arange(0, 201, 20))
plt.xlabel("Time [min]"); plt.ylabel("Flow [kg/s]"); plt.title("Figure 4: F5 and F3H2 profiles")
plt.legend(); plt.grid(True, linestyle='-', alpha=0.6)
